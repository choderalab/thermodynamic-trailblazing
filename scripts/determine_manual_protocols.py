...
def generate_manual_protocol(n_states, phase_name):
    ...
    windows = np.linspace(1.0, 0.0, n_sterics_states).tolist()
    protocol['lambda_sterics'] = [1.0] * (n_restraint_states + n_electrostatics_states) + windows
...
def determine_all_manual_protocols(print_n_states_info=True):
        ...
        for system_name, system_protocols in distance_protocols.items():
            ...
            for ligand_id, ligand_protocols in system_protocols.items():
                for phase_name, phase_protocols in ligand_protocols.items():
                    # There should be 3 replicates for each system.
                    assert len(phase_protocols) == 5, f'{protocol_name}-{phase_name}: {len(phase_protocols)}'
...
if __name__ == '__main__':
    determine_all_manual_protocols(print_n_states_info=True)
